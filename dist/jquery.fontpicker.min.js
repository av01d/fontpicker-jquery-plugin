...
g.intersectionRatio&&(e.unobserve(g.target),b=a(g.target),c.loadFont(b.data("font-type"),b.data("font-family")),b.css("fontFamily","'"+b.data("font-family")+"'"))})});a("li:not(.fp-divider):visible",this.$results).each(function(){e.observe(this)})}},toggleModal:function(c){c||(c=this.$modal.is(":visible")?"hide":"show");if("hide"==c)a(".fp-fav",this.$results).remove(),this.$modal.css("display","none"),a(".fp-modal-backdrop",this.$element).remove(),a(this.options.parentElement).removeClass("fp-modal-open");
else{this.fontActive=null;c=this.$original.val();a(this.options.parentElement).addClass("fp-modal-open");this.$element.append(a('<div class="fp-modal-backdrop">').on("click",function(){a(".fp-close",this.$modal).trigger("click")}));this.$modal.css("display","flex");this.options.lazyLoad&&this.lazyLoad();var b=n("favs");this.favFonts=b?b.split(","):[];this.getFavorites(this.favFonts);c?(c=this.fontSpecToComponents(c),b=a("[data-font-family='"+c.family+"']",this.$results),b.trigger("mouseenter").trigger("click"),
a("span.fp-pill",b).removeClass("checked"),a("span[data-font-weight='"+c.weight+"']",b).addClass("checked"),c.italic&&a("span.italic",this.$results).addClass("checked"),m(b[0])):this.$results.scrollTop(0);this.$results.focus()}},applyFilter:function(){var c=this.$lang.val(),b=this.$search.val().trim(),e=[];a(".fp-category",this.$filter).each(function(){a(this).hasClass("checked")&&e.push(a(this).data("category"))});n("lang",""===c?!1:c);n("cats",e.join(","));for(var g in this.allFonts)for(var d in this.allFonts[g]){var f=
this.allFonts[g][d],k=f.subsets?f.subsets.split(","):[],h=a("li[data-font-family='"+d+"']",this.$results);f=f.category||"other";""!=c&&-1==k.indexOf(c)||-1==e.indexOf(f)||""!=b&&-1==d.toLowerCase().indexOf(b.toLowerCase())?h.hide():h.show()}},getFilterUI:function(){var c=this;this.$filter=a('<div class="fp-filter">');this.$search=a("<input>",{"class":"fp-search",type:"text",placeholder:this.dictionary.search}).on("keyup",function(){c.applyFilter()});var b=['<option value="">'+this.dictionary.allLangs+
