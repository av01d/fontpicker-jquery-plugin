...
b.replace(/ /g,"+")+":"+this.options.googleFonts[b].variants+"&display=swap";c("head").append(c("<link>",{href:d,rel:"stylesheet",type:"text/css"}));break;case "local":this.options.debug&&console.log("Loading local font "+b),"FontFace"in window?(new FontFace(b,"url('"+this.options.localFontsUrl+b+".woff')")).load().then(function(e){document.fonts.add(e)}):c("head").append("<style> @font-face { font-family:'"+b+"'; src:local('"+b+"'), url('"+this.options.localFontsUrl+b+".woff') format('woff'); } </style>")}},
showSample:function(a){c(".fp-sample",this.$element).css({fontFamily:"'"+a.data("font-family")+"'",fontStyle:a.data("font-italic")?"italic":"normal",fontWeight:a.data("font-weight")||400})},keyDown:function(a,b){function d(e){e.preventDefault();e.stopPropagation()}b=c("li.fp-active:visible",this.$results);if(49<=a.keyCode&&57>=a.keyCode||97<=a.keyCode&&105>=a.keyCode)d(a),c(".fp-pill[data-font-weight="+100*(a.keyCode-(97<=a.keyCode?96:48))+"]",b).trigger("click");else switch(a.keyCode){case 73:d(a);
