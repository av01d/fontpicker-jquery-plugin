...
d={},e;var f=(f=/(?:^|; )jqfs=([^;]*)/.exec(document.cookie))?decodeURIComponent(f[1]):null;if(null!==f)for(e in f=f.split("||"),f)try{var h=f[e].split("|",2);d[h[0]]=h[1]}catch(l){}if(1===arguments.length)return d[n];null===a||!1===a?delete d[n]:d[n]=a;f=[];for(e in d)f.push(e+"|"+d[e]);b.setTime(b.getTime()+31536E6);b="; expires="+b.toGMTString();document.cookie="jqfs="+encodeURIComponent(f.join("||"))+"; path=/; SameSite=Lax"+b},x={ABeeZee:{category:"sans-serif",variants:"400,400i",subsets:"latin"},
...
"&display=swap";this.options.debug&&console.log("Loading Google font "+b+" from "+a);c("head").append(c("<link>",{href:a,rel:"stylesheet",type:"text/css"}));break;case "local":this.options.debug&&console.log("Loading local font "+b),c("head").append("<style> @font-face { font-family:'"+b+"'; src:local('"+b+"'), url('"+this.options.localFontsUrl+b+".woff') format('woff'); } </style>")}},showSample:function(a){c(".fp-sample",this.$element).css({fontFamily:"'"+a.data("font-family")+"'",fontStyle:a.data("font-italic")?
...
b){a=c(b);c("li.fp-hover",this.$results).removeClass("fp-hover");a.addClass("fp-hover");this.loadFont(a.data("font-type"),a.data("font-family"));this.showSample(a)},click:function(a,b){var d=c(b),e=this,f=d.data("font-type"),h=d.data("font-family"),l=d.data("font-italic")||!1,k=d.data("font-weight")||400,p=c("li[data-font-family='"+h+"']",this.$results),v=(a=m("favs"))?a.split(","):[];c("li.fp-active",this.$results).removeClass("fp-active").find(".fp-variants,.fp-btns").remove();d.addClass("fp-active");
a=c('<div class="fp-btns">');b=-1!=v.indexOf(f+":"+h);a.append(c('<span class="fp-favorite'+(b?" checked":"")+'"></span>').on("click",function(g){g.stopPropagation();g=v.indexOf(f+":"+h);c(this).is(".checked")?-1!=g&&v.splice(g,1):-1==g&&v.push(f+":"+h);c(this).toggleClass("checked");m("favs",v.join(","))}),c('<button type="button" class="fp-btn apply">').html(this.dictionary.select).on("click",function(g){g.stopPropagation();l=d.data("font-italic");k=d.data("font-weight")||400;g=h;e.options.variants&&
(g+=":"+k+(l?"i":""));e.$select.css({fontFamily:"'"+h+"'",fontStyle:l?"italic":"normal",fontWeight:k}).find(".fp-fontspec").html(g);e.$original.val(g).change();if(e.options.onSelect)e.options.onSelect({fontType:f,fontFamily:h,fontStyle:l?"italic":"normal",fontWeight:k,fontSpec:g});e.toggleModal("hide");if(e.options.nrRecents){g=m("recents");var r=d.data("font-type")+":"+h;g=g?g.split(","):[];-1==g.indexOf(r)&&g.unshift(r);g=g.slice(0,e.options.nrRecents);m("recents",g.join(","))}}));a.appendTo(d);
a=this.allFonts[f][h];var q=a.variants?a.variants.split(","):[];if(this.options.variants&&1<q.length){a=c('<div class="fp-variants">');b=!1;for(var w=0;w<q.length;w++){if(/i$/.test(q[w])){b||(b=!0);continue}let g=q[w],r=+g.replace(/i$/,"");0<w&&a.append(" ");c('<span data-font-weight="'+r+'" class="fp-pill weight'+(k==r?" checked":"")+'">').text(g).on("click",function(G){G.stopPropagation();-1==q.indexOf(r+"i")?(c(".fp-pill.italic",d).removeClass("checked").css("display","none"),l=!1,d.data("font-italic",
l)):c(".fp-pill.italic",d).css("display","");c("span.fp-pill.weight",d).removeClass("checked");c(this).addClass("checked");p.data("font-weight",r);e.showSample(d)}).appendTo(a)}b&&(a.append(" "),c('<span class="fp-pill italic '+(l?" checked":"")+'">').css("display",-1==q.indexOf(k+"i")?"none":"").html("italic").on("click",function(g){g.stopPropagation();l=!l;c(this).toggleClass("checked");p.data("font-italic",l);e.showSample(d)}).appendTo(a));d.append(a)}else 1==q.length&&/i$/.test(q[0])&&p.data("font-italic",
...
b[d]).text(b[d]).on("click",function(){c(this).toggleClass("checked");a.applyFilter()}).appendTo(this.$filter)},getFontsList:function(){function a(h,l){var k=b.allFonts[h][l],p="";if(k.category||k.variants)p=[],k.category&&p.push(k.category),b.options.variants&&k.variants&&(k=k.variants.split(",").length,1<k&&p.push(k+" "+b.dictionary.styles)),p=" <small>"+p.join(", ")+"</small>";f=c("<li>",{"data-font-type":h,"data-font-family":l}).html(l+p);d.append(f[0])}var b=this,d=c(document.createDocumentFragment()),
...
b.split(","):[];b=b.slice(0);for(var d=0;d<a.length;d++)-1==b.indexOf(a[d])&&b.push(a[d]);a=c(document.createDocumentFragment());var e=null;for(d=0;d<b.length;d++){var f=b[d].split(":");fontType=f[0];fontFamily=f[1];if(font=this.allFonts[fontType][fontFamily])f=c("[data-font-family='"+fontFamily+"']",this.$results),0<f.length&&(e=f.clone().addClass("fp-fav"),a.append(e[0]))}null!==e&&(a.prepend(c('<li class="fp-fav fp-divider">'+this.dictionary.favFonts+"</li>")[0]),this.$results.prepend(a))},setupHtml:function(){var a=
